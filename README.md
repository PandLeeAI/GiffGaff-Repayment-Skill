...
把飞书《GiffGaff小白卡退款教学》消化、简化、优化成的 **Skill**，让任意 agent 加载后，交互式引导用户一步步完成 giffgaff 海外停号后的维权：
...
**诉求由用户自己多选决定（保号 / 退款 / 赔偿 / 转网，可多选）**，agent 据此动态拼装英文范文、动态调整流程，并支持与官方、通信局**多轮流程**。
...
1. 把本目录复制到任意 agent 的技能目录（用户级 `/skills/giffgaff-repayment/` 或项目级 `skills/giffgaff-repayment/`），或直接 `git clone` 后放入技能目录。
